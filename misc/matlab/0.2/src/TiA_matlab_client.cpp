...
void ssc_getData(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  try
  {
    ...
    if (!client->receiving())
    {
     ...
     mxSetField(infoStruct,0,"sampleNr",mxCreateDoubleScalar(packet->getPacketID()));
     
     unsigned int cellIndex = 0;
     for(map<uint32_t, tia::Signal >::iterator it = sig_types_map->begin(); it != sig_types_map->end();  it++)
     {
       try {
